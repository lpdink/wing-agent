...
- **粗定位 = 按下空白轨道**：按比例跳转（手柄中心对齐指针），一次手势能到任意位置（1 指针行 = `max_scroll / travel` 行，长会话里是几十上百行，适合「跳」不适合「找」）。
- **细定位 = 拖动手柄**：**1 指针行 = 1 个滚轮档 = `WHEEL_SCROLL_LINES`(3) 行**，以按下点为锚做相对移动（按下后不松手，等于「先跳后微调」）。指针行会夹进轨道范围，所以拖出轨道不会加速、也不会失控；释放结束拖动。
- **为什么不能「又细又一把到底」**：`max_scroll = travel × step`（`travel = 轨道行数 - 手柄行数`）。终端只有行、没有像素，轨道最长就是 chat band 的高度（整屏也就几十行），所以拖动想细（step=3）就必然一趟只覆盖 `travel × 3`（40 行轨道 ≈ 110 行内容）——超出部分交给「按一下空白轨道跳过去，再拖手柄微调」。手柄长度本身就是 `视口 / 内容` 的比例（显示位置用），再缩短它并不会换来行程，行程的上限是轨道高度。
...
- **滚动条列不是内容**：只要这一帧画了滚动条，选区映射（`App::off_the_bar`）就把落点夹到它左边一列，`ChatView::set_content_width` 同时把**整段**的可选宽度收窄到这一列之前——否则复制出的文本会混进 `│`（以及它前面那串本行补位空格：`trim_end` 会停在 `│` 上）、整段高亮也会把滚动条反显。内容不溢出（无滚动条）时整列照常可选。
...
- **重绘节流**：指针移动与拖动事件走 `chat_dirty`（受 16ms 帧门限），滚轮 / 按下 / 释放这类离散手势走 `input_dirty`（立即重绘）；无状态变化的事件不设任何 dirty 标志。拖动的步长与滚轮一致（`WHEEL_SCROLL_LINES`），所以「拖手柄」和「在滚动条上滚轮」手感相同。
