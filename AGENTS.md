...
**Protocols.** HTTP for lifecycle / queries / mutations (~20 RPC-style endpoints); WebSocket (`/ws`) for real-time ReAct event streaming only. Session creation is decoupled from the WS handshake — clients create a session via HTTP, then subscribe to events. API key auth is opt-in at the Gateway (HTTP headers / WS query param); TLS is delegated to a reverse proxy.
