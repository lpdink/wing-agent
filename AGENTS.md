...
**Unified event log.** `history.jsonl` is the single source of truth — a mixed log of Message records (LLM-context projection) and event records (`role="event"`), all sharing chain topology. Two-level commit: streaming deltas commit to RAM (`EventJournal` — live per-packet broadcast with multi-packet coalescing for replay) and are merged into a Message record at turn close; `persist=true` events (diff, metrics, ask, interrupted, tool_call_result…) hit the log the moment they complete. Interrupt/max-tokens turns commit their partial content (unfinished tool calls are dropped — unverifiable args + dangling tool_use); providers expose a caller-held stream accumulator for post-cancel snapshots and capture stop_reason. rewind/fork/compact work on events for free via chain order; mid-turn subscribers get `messages + events + in_flight` in SyncSessionEvent for a view identical to subscribing from the start.
...
│   ├── react_loop.py             ReAct main loop: drain → hook → LLM → tools → commit; partial commit on interrupt
...
│   ├── event_sink.py             AgentEventSink — single event emission outlet + persist split
│   ├── event_journal.py          EventJournal — RAM commit layer, transient delta coalescing
...
├── event/                        Event types (base, react, state_change, query_response) + EVENT_TYPES registry + persist flags
