...
impl App {
    ...
    ///
    /// Left press / drag / release drive the in-app text selection, in
    /// whichever region the press landed in (see [`mouse_press`]): the chat
    /// band keeps its drag-to-copy contract, the composer adds click-to-place-
    /// cursor. A press outside both regions (status bar, popups) is ignored,
    /// exactly like `Moved` (hover, taken over by the scrollbar change) and
    /// horizontal wheel.
    ///
    /// [`mouse_press`]: App::mouse_press
    fn handle_mouse(&mut self, mouse: crossterm::event::MouseEvent) -> MouseOutcome {
        ...
        match mouse.kind {
            ...
            MouseEventKind::Up(MouseButton::Left) => self.mouse_release(mouse.column, mouse.row),
    ...
    ///
    /// The composer is checked first — it is the only region that reacts to a
    /// plain click (it places the cursor), and it is guarded by the modal
    /// panels (see [`Self::composer_pointer_blocked`]). Everything else falls
    /// through to the chat band, whose press is ignored outside its rect.
    fn mouse_press(&mut self, column: u16, row: u16) -> MouseOutcome {
        if self.composer_contains(column, row) {
            return self.composer_press(column, row);
        }
    ...
    /// and the command candidate popup take the keyboard while they are up:
    ...
    /// such a panel would race that flow, so composer pointer interaction is
    /// ignored outright. The chat band's drag selection and the wheel are
    /// **not** affected (rolling history while a panel is open has to keep
    /// working).
    fn composer_pointer_blocked(&self) -> bool {
        !self.ask_panels.is_empty()
            || self.ask_selections.front().is_some()
            || self.model_panel.is_some()
            || self.popup.active.is_active()
    }
    ...
    fn composer_release(&mut self, column: u16, row: u16) -> MouseOutcome {
        ...
        // The same gate as the chat band: the character under the pointer is
        // only included when the pointer really travelled.
